...
-- show each jobs salary next to the overall market media:
...
-- 2 subquery in FROM
-- Stage only jobs trhat are remote before aggregating to determine the remote median salary per job
...
-- Compare hwo much (or less) remote roles pay compared to onsite roles for each job title.
